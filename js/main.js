...
───────────────────────────────────────────────────────── */
// Ngày giờ cưới thực tế: 09:00 ngày 12 tháng 04 năm 2026
const WD_DATE = new Date(2026, 3, 12, 9, 0, 0).getTime();

function updateCountdown() {
  const diff = WD_DATE - new Date().getTime();
  const timerEl = document.getElementById('wedding-timer');
  if (!timerEl) return;
  ...
  if (diff > 0) {
    ...
    timerEl.innerHTML = '<p style="font-family:Great Vibes,cursive;font-size:2.4rem;color:#C8102E;margin:10px 0">Hôm nay là ngày trọng đại! 🎊</p>';
  }
}
